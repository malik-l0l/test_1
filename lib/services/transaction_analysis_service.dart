...
class TransactionAnalysisService {
  ...
  static List<int> findSettlementPoints(List<PeopleTransaction> transactions) {
    ...
    for (int i = transactions.length - 1; i >= 0; i--) {
      ...
      // If balance becomes zero, check if the next transaction (i-1) breaks the settlement
      if (runningBalance == 0.0 && i > 0) {
        // The separator should appear before the next transaction that breaks settlement
        // So we mark index (i-1) as needing a separator
